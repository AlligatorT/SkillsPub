...
The first publish is an interactive maintainer 2FA cutover. Configure trusted publishing and provenance only after that flow is proven; do not weaken 2FA to automate v0.2.0.

### GitHub

- Admin permission for `AlligatorT/SkillsPub`, including repository visibility changes.
- Permission to push the `v0.2.0` tag and create a GitHub Release.
...
- The npm tarball URL and SHA-256 from the published immutable v0.2.0 artifact.
...
5. Publish the exact verified tarball with maintainer 2FA: `npm publish ./skillspub-0.2.0.tgz --access public`.
6. Verify from a clean temporary directory with `npm view skillspub@0.2.0` and `npx skillspub@0.2.0 targets --json`. npm versions are immutable; if verification fails, deprecate the broken version with a clear message, fix forward with a new patch version, and do not reuse `0.2.0`.
7. Tag the recorded commit as `v0.2.0`, push the tag, and create a GitHub Release that includes the recorded tarball SHA-256. If this step fails, leave npm intact, repair GitHub permissions or metadata, and create the tag/release from the same recorded commit.
...
9. Confirm npm, GitHub, and Homebrew all resolve to v0.2.0 from the recorded commit, then unfreeze development.
