...
- inspect、scan、ls 与打开 TUI 严格只读。setup、apply 与 reconcile 先展示计划；写 Harness 配置前复核原内容、原子写入并重新验证。未知配置结构拒绝写入。支持级别为 `managed`、`discoverable`、`unsupported`，首个完整 Managed Adapter 是 Pi。
